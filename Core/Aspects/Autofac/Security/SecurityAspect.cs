...
public class SecurityAspect : MethodInterception
{
    ...
    protected override void OnBefore(IInvocation invocation)
    {
        ...
        {
            throw new AuthenticationException("You are not authentication"); // Login değilsin
        }
        var claims = user.Claims.ToList().Where(c => c.Type == System.Security.Claims.ClaimTypes.Role).Select(c => c.Value).ToList();
        if (!roles.Where(rol => claims.Contains(rol)).Any()) // rollerden hiçbiri yoksa 
        {
            throw new AuthorizationException(role,"You are not authorized"); //Loginsin ama yetkin yok
        }
    }
}